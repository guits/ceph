...
from cephadmlib.node_proxy.main import NodeProxy, NodeProxyFetchOobError
...
class NodeProxyManager(Thread):
    ...
    def init(self) -> None:
        ...
        if status != 200:
            msg = f"Couldn't load oob tool details: {status}, {result}"
            logger.debug(msg)
    ...
    def loop(self) -> None:
        while not self.stop:
            try:
                ...
                logger.info('No out of band tool details could be loaded. '
