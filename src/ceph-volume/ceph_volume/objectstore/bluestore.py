...
from ceph_volume.util import system, prepare as prepare_utils
    ...
    def add_objectstore_opts(self):
        ...
        if self.wal:
            self.osd_mkfs_cmd.extend(
                ['--bluestore-block-wal-path', self.wal]
            )
            system.chown(self.wal)

        if self.db:
            self.osd_mkfs_cmd.extend(
                ['--bluestore-block-db-path', self.db]
            )
            system.chown(self.db)
