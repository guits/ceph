...
class LvmBlueStore(BlueStore):
    ...
    def prepare_dmcrypt(self, key, block, db, wal, tags):
        ...
        self.block = self.luks_format_and_open(key, block, 'block', self.tags)
        self.wal = luks_format_and_open(key, wal, 'wal', self.tags)
        self.db = luks_format_and_open(key, db, 'db', self.tags)

    def luks_format_and_open(key, device, device_type, tags):
        """
