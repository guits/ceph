...
class RawOsdCryptMappers:
    ...
    def mapper_name_for_role(self, role: str) -> str:
        return self._mapper_names[role]

    def mapper_name_for_backing(self, backing_device_path: str, role: str) -> str:
        return self.mapper_name_for_role(role)

    def mapper_path_for_backing(self, backing_device_path: str, role: str) -> str:
        return '/dev/mapper/%s' % self.mapper_name_for_role(role)

    def mapper_paths(self) -> Tuple[str, str, str]:
        return (
            self.mapper_path_for_backing(self.block_device, 'block'),
            self.mapper_path_for_backing(self.db_device, 'db') if self.db_device else '',
            self.mapper_path_for_backing(self.wal_device, 'wal') if self.wal_device else '',
        )
    ...
    def _close_crypt_for_role(self, role: str) -> None:
        encryption_utils.dmsetup_remove(
            self.mapper_name_for_role(role),
