    ...
    def find_free_port(self) -> None:
        ...
        while self.server_port <= max_port:
            try:
                ...
                self.mgr.log.debug(f'{self.name} using {self.server_port}')
                return
            except PortAlreadyInUse:
                self.server_port += 1
        self.mgr.log.error(f'{self.name} could not find free port in range {max_port - 150}-{max_port} and failed to start')